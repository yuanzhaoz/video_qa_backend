...
class VQA():
    def __init__(self, pretrain_path):
        ...
        print(f"Length of Answer Vocabulary: {len(self.a2id)}")

        # Model
        self.model = MMT_VideoQA(
            feature_dim=self.args.feature_dim,
            word_dim=self.args.word_dim,
            N=self.args.n_layers,
            d_model=self.args.embd_dim,
            d_ff=self.args.ff_dim,
            h=self.args.n_heads,
            dropout=self.args.dropout,
            T=self.args.max_feats,
            Q=self.args.qmax_words,
    ...
    def response(self, video_path, question_txt):
        ...
        question = torch.tensor(
            self.model.module.bert.bert_tokenizer.encode(
                question_txt,
                add_special_tokens=True,
                padding="max_length",
                max_length=self.args.qmax_words,
                truncation=True,
            ),
            dtype=torch.long,
        )
        ...
        with torch.no_grad():
            ...
            # Pad Video
            if len(video) < self.args.max_feats:
                video = video[: self.args.max_feats]
                video_len = len(video)
                if len(video) < self.args.max_feats:
                    video = torch.cat(
                        [video, torch.zeros(self.args.max_feats - len(video), video.shape[1]).cuda()]
                    )
            else:
                sampled = []
                for j in range(self.args.max_feats):
                    sampled.append(video[(j * len(video)) // self.args.max_feats])
...
# def vqa(video_path, question_txt):
#     # args
#     args = get_args()
